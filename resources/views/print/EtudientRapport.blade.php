    ...
    <section class="wrapper-invoice">
        ...
        <div class="invoice">
            ...
            <table class="header-table" style="border-collapse: collapse;">
                <th>
                <td>
                    <h2 style="text-decoration: underline; text-transform: uppercase;">Liste des @if(Session::get('type') == "1" OR Session::get('type') == "2") Elèves @else Etudiants @endif Année scolaire {{$optionAnneescolaire}}</h2>
                </td>
                </th>
            </table>
            <br>
            <div class="invoice-body">
                ...
                <table class="table">
                    <thead>
                        <td style="color: #dcdcdc;background-color:rgb(73, 64, 64);padding: 5px 0px;text-align: center;" colspan="4">{{$Formation->nom}}</td>
                    </thead>
                    <thead>
                        ...
                        <td style="background-color:rgb(233, 233, 233);padding: 7px 0px;text-align: center;">@if(Session::get('type') == "4") Approche de formation @elseif(Session::get('type') == "1" OR Session::get('type') == "2") Classe @else Formation @endif</td>
                        <td style="background-color:rgb(233, 233, 233);padding: 7px 0px;text-align: center;">Année scolaire</td>
                    </thead>
                    @foreach($FormationParticipants as $FormationParticipant)
                    @if($Formation->id == $FormationParticipant->formation_id)
                    <tr>
                        <td style="font-weight: 200;font-size: 14px;text-align: center;padding: 2px 0px;">{{$FormationParticipant->Participant->nom}}</td>
                        <td style="font-weight: 200;font-size: 14px;text-align: center;padding: 2px 0px;">{{$FormationParticipant->Participant->prenom}}</td>
                        <td style="font-weight: 200;font-size: 14px;text-align: center;padding: 2px 0px;">{{$FormationParticipant->Formation->nom}}</td>
                        <td style="font-weight: 200;font-size: 14px;text-align: center;padding: 2px 0px;">{{$FormationParticipant->anneeScolaire}}</td>
                    </tr>
                    @endif
                    @endforeach
                </table>
